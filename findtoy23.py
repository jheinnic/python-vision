...
TICKET_CANNY_THRESH_ONE=250
TICKET_CANNY_THRESH_TWO=700
...
class SiftImage:
   def __init__(self, label, selfImg, filePath, siftInst):
      ...
      self.edgeImg = cv2.Canny(selfImg, TICKET_CANNY_THRESH_ONE, TICKET_CANNY_THRESH_TWO)
      (self.keypoints, self.descriptors) = siftInst.detectAndCompute(self.selfImg, None)
      (self.edgeKeypoints, self.edgeDescriptors) = siftInst.detectAndCompute(self.edgeImg, None)
   ...
   def getEdgeKeypoints(self):
      return self.edgeKeypoints

   def getEdgeDescriptors(self):
      return self.edgeDescriptors
...
class LetterMatcher:
   ...
   def getMatchScore(self, searchInImg):
       return 0
...
class TicketCell:
   def __init__(self, cellImg, cellSifter, cellLabel, cellFilePath, matchFileTemplate):
      ...
      print('Cell is blank if (%d+9) or %d >= %d' % ((percentiles[0.15]+9), percentiles[0.15], percentiles[0.3]))
   ...
   def visualizeByOverlay(self, cellImg, exemplarImg, resultRaw, topLeft, letter, fileTemplate):
      ...
      for ii in range(topLeft[1],btmRight[1]):
         for jj in range(topLeft[0],btmRight[0]):
            if exemplarImg[ii-topLeft[1]][jj-topLeft[0]] < 128:
                mask[ii][jj][0] = 255
            else:
                mask[ii][jj][1] = 255
...
class CrosswordTicket:
   ...
   def locateCrossword(self, queryObj):
       kpQ = queryObj.getEdgeKeypoints()
       desQ = queryObj.getEdgeDescriptors()
   ...
   def visualizeCrosswordLocation(self, sampleDir):
      ...
      M = self.M
      kpT = self.ticketImg.getKeypoints()
      kpQ = self.queryObj.getEdgeKeypoints()
      queryImg = self.queryObj.getImage()
      queryEdgeImg = self.queryObj.getEdgeImage()
      ticketImg = self.ticketImg.getImage()
      ...
      flannEdgeMatchImg = cv2.drawMatches(queryEdgeImg, kpQ, decoratedImg, kpT, self.goodMatches, None, **draw_params)
      cv2.imwrite(sampleDir + '/flannWarp.tif', flannMatchImg)
      cv2.imwrite(sampleDir + '/flannEdgeWarp.tif', flannEdgeMatchImg)
      return True
